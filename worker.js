 ...
 * نسخه Cloudflare Workers - تبدیل شده از Python
 * تمام قابلیت‌ها حفظ شده
 */
...
async function saveMessage(env, userId, sectionKey, msgType, content, fileId = null, fileName = null) {
  ...
  const result = await db.exec(`
    ...
    VALUES (${userId}, '${sectionKey}', '${msgType}', '${(content || '').replace(/'/g, "''")}', '${fileId || ''}', '${fileName || ''}', '${now}', 'pending')
    RETURNING msg_id
  `);
  
  // به‌روزرسانی تعداد پیام‌ها
  await db.exec(`
    UPDATE users SET total_messages = total_messages + 1 WHERE user_id = ${userId}
  `);
...
async function getInboxMessages(env, adminId, sectionKey = null, limit = 20) {
  ...
  if (isSuperAdmin(adminId)) {
    ...
    if (sectionKey) {
      ...
      const placeholders = userSections.map(() => '?').join(',');
      // برای سادگی، از IN با پارامتر استفاده نمی‌کنیم
...
async function getAllUsers(env, limit = 50) {
  const db = env.DB;
  const result = await db.exec(`
...
async function handleStart(env, message) {
  ...
  if (isSuperAdmin(user.id)) {
    await upsertUser(env, { ...user, id: user.id });
  }
  
  // بررسی مسدود بودن
  const userData = await getUserById(env, user.id);
  ...
  if (isSuperAdmin(user.id)) {
    const text = `👑 <b>پنل سوپر ادمین</b>\n━━━━━━━━━━━━━━━\nشما دسترسی کامل به همه بخش‌ها دارید.\n\n📌 <b>دستورات ویژه:</b>\n/set_admin - افزودن ادمین جدید\n/remove_admin - حذف ادمین\n/super_panel - پنل مدیریت کامل\n/all_messages - مشاهده همه پیام‌ها\n/activity - گزارش فعالیت‌ها`;
    await sendMessage(chatId, text, 'HTML', createSuperAdminMenu());
    return;
  }
...
async function handleSelectSection(env, callback) {
  ...
  // ذخیره بخش انتخاب شده در یک متغیر موقت (برای استفاده در step handler)
  // در Cloudflare Workers باید از state استفاده کنیم
  // برای سادگی، از یک Map ساده استفاده می‌کنیم
  // در اینجا فقط پیام رو ذخیره می‌کنیم و در handleUserMessage استفاده می‌کنیم
  userSectionCache.set(chatId, sectionKey);
}

// کش برای ذخیره بخش انتخاب شده
  ...
  for (const adminId of adminIds) {
    try {
      ...
      if (message.text) {
        ...
        await forwardMessage(adminId, message.chat.id, message.message_id);
      }
  ...
  if (msg.file_id) {
    try {
      if (msg.message_type === 'photo') {
        await sendPhoto(chatId, msg.file_id);
...
async function handleReply(env, callback) {
  ...
  // ذخیره msgId برای مرحله بعد
  pendingReplies.set(chatId, msgId);
  
  await answerCallback(callbackId);
}

// کش برای پاسخ‌های در انتظار
...
async function handleUsers(env, message) {
  ...
  const users = await getAllUsers(env, 20);
  if (users.length === 0) {
...
// ============================================
// بخش‌های دیگر (broadcast, section_stats, debug, fix)
// ============================================
...
async function handleDebug(env, message) {
  ...
  const isSuperStatus = isSuperAdmin(userId);
  const userSections = getUserSections(userId);
  const stats = await getStats(env);
  
  let text = `🔍 <b>اطلاعات دیباگ ربات</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n🆔 <b>شناسه شما:</b> <code>${userId}</code>\n👑 <b>سوپر ادمین:</b> ${isSuperStatus ? '✅ بله' : '❌ خیر'}\n👤 <b>ادمین:</b> ${isAdminStatus ? '✅ بله' : '❌ خیر'}\n📋 <b>بخش‌های شما:</b> ${userSections.map(s => getSectionName(s)).join(', ') || 'هیچکدام'}\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n📊 <b>آمار کلی:</b>\n👥 کاربران: ${stats.total_users}\n💬 کل پیام‌ها: ${stats.total_messages}\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n📋 <b>آمار بخش‌ها:</b>\n`;
  
  for (const key in stats.section_stats) {
    const name = SECTIONS[key].name;
    const stat = stats.section_stats[key];
    const isAdminThis = userSections.includes(key) ? '⭐' : '  ';
...
async function handleCallbackQuery(env, callback) {
  ...
  if (data.startsWith('user_info_')) {
    // برای سادگی، اطلاعات کاربر رو نمایش میدهیم
    ...
    if (request.method === 'POST') {
      try {
        ...
        if (update.message) {
          ...
          } else if (text === '/block' || text.startsWith('/block')) {
            // برای سادگی، از دستور block استفاده نمیکنیم
            await sendMessage(msg.chat.id, '🚫 از پنل سوپر ادمین برای بلاک استفاده کنید.');
          } else if (text === '/unblock' || text.startsWith('/unblock')) {
            await sendMessage(msg.chat.id, '✅ از پنل سوپر ادمین برای رفع بلاک استفاده کنید.');
          } else {
            // پیام معمولی
            // بررسی اینکه آیا پیام به یک دستور خاص نیست
            if (!text.startsWith('/')) {
              // بررسی اینکه کاربر بخشی انتخاب کرده یا نه
              const chatId = msg.chat.id;
              if (userSectionCache.has(chatId)) {
                await handleUserMessage(env, msg);
              } else {
                // پیام معمولی بدون انتخاب بخش
                await sendMessage(chatId, '⚠️ لطفاً ابتدا بخش مورد نظر را انتخاب کنید.\nاز /start استفاده کنید.');
              }
            }
          }
